...
namespace Repository.Repositories.Classes
{
    public class DebitRepository: Repository<DebitEntity>
    {
        ...
        {
            var debits = _context.Debits.Where(e => e.employeeId == employeeId && e.active && e.remainingPays > 0 && e.fkdebit_type.type == "F");
            return debits.ToList();
        }

        public IEnumerable<DebitEntity> selectPaymentDebitsByEmployee(long employeeId)
        {
            var debits = _context.Debits.Where(e => e.employeeId == employeeId && e.active && e.remainingPays > 0 && e.fkdebit_type.type == "P");
            return debits.ToList();
        }

        public IEnumerable<DebitEntity> selectAmortizationDebitsByEmployee(long employeeId)
        {
            var debits = _context.Debits.Where(e => e.employeeId == employeeId && e.active && e.remainingPays > 0 && e.fkdebit_type.type == "A");
            return debits.ToList();
        }
    }
}
